object December3 {
  ...
  case class Point(x: Int, y: Int) {
    ...
    def next(direction: String): Point = direction match {
      case "D" => down()
      case "U" => up()
      case "L" => left()
      case "R" => right()
  ...
  def toPath(directions: List[String]): List[Point] = {

    def normalize(direction: String): List[String] = {
      val r = """([UDRL])(\d+)""".r
      direction match {
        case r(d, count) => List.fill(count.toInt)(d)
      }
    }
    ...
    directions
      .flatMap(normalize)
      .foldLeft(List(Zero)) {
        case (p, d) => p.head.next(d) :: p
      }
      .reverse
  }

  def intersection(path1: List[Point], path2: List[Point]): List[Point] =
    path1.intersect(path2).filterNot(_ == Zero)

  def minDistance(path1: List[Point], path2: List[Point]): Option[Point] =
    intersection(path1, path2).minByOption(_.distance)

  def minSteps(path1: List[Point], path2: List[Point]): Option[(Int, Point)] =
    intersection(path1, path2)
      .flatMap { p =>
        for {
          step1 <- findPosition(p, path1)
          step2 <- findPosition(p, path2)
        } yield (step1 + step2) -> p

      }
      .minByOption(_._1)

  def findPosition(point: Point, path: List[Point]) =
    path.zipWithIndex.find(_._1 == point).map(_._2)
  ...
  def main(args: Array[String]): Unit = {
    ...
    println(minDistance(path1, path2).map(_.distance).getOrElse(""))
    println(minSteps(path1, path2).map(_._1).getOrElse(""))
  }
